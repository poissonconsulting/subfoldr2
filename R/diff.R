...
#' @family compare
#' @export
sbf_diff_datas <- function(
  x_name = ".*", sub = sbf_get_sub(), main = sbf_get_main(),
  archive = 1L, recursive = FALSE, include_root = TRUE, exists = NA) {
  chk_lgl(exists)
  ...
  if(!length(all_file_names)) return(structure(list(), .Names = character(0)))
  
  all_file_names <- sort(all_file_names)
  
  diff <- lapply(all_file_names, FUN = diff_data,
