...
test_that("spatial", {
  ...
  expect_error(
    check_spatial(y),
    "^`y` must inherit from S3 class 'sf', not S3 class 'numeric'[.]$"
  )
...
test_that("plot", {
  ...
  expect_error(
    sbf_save_plot(y),
    "^`x` must inherit from S3 class 'ggplot', not S3 class 'numeric'[.]$",
    class = "chk_error"
  )
...
test_that("load_rdss_recursive() informs user on which folders were dropped.", {
  withr::local_options(sbf.quiet = FALSE)
  sbf_reset()
  sbf_set_main(file.path(withr::local_tempdir(), "output"))
  withr::defer(sbf_reset())
  ...
  expect_snapshot(
    # to deal with multiline message
    expect_identical(
  ...
  expect_warning(
    sbf_load_tables_recursive(drop = "fake-dir", quiet = TRUE),
    "No files or folders matched `drop`, so no files were dropped\\."
  )
  ...
  expect_snapshot(
    sbf_load_tables_recursive(drop = "sub1") %>%
      dplyr::mutate(file = gsub(".*tables.", "", file))
  )
...
test_that("notes argument is saved for numbers and strings", {
  withr::with_tempdir({
    ...
    expect_identical(
      num_meta,
      list(report = TRUE, tag = "", notes = "num note")
    )

    str_meta <- yaml::read_yaml(file.path(sbf_get_main(), "strings", "s.yaml"))
    expect_identical(
      str_meta,
      list(report = TRUE, tag = "", notes = "str note")
    )
...
test_that("plot with geometry layers is saved properly.", {
  ...
  expect_equal(readxl::read_xlsx(xlsx_file, sheet = "1_1_sf"), tibble())

  # one informative column in the data but not the layers
  shp <- sf::st_as_sf(
    data.frame(x = c(1, 2), y = c(10, 15), let = c("a", "b")),
    coords = c("x", "y")
  )
  ...
  expect_equal(readxl::excel_sheets(xlsx_file), c("1_0_data", "1_1_sf"))
  expect_equal(
    readxl::read_xlsx(xlsx_file, sheet = "1_0_data"),
  ...
  expect_equal(
    readxl::read_xlsx(xlsx_file, sheet = "1_1_sf"),
