...
Saves an sf tbl with at least one row for which the first column (not a
geometry) is unique with no missing values and only one geometry column
which must have a defined projection.
