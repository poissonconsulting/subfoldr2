...
Recursively loads all the default data from the plots with names
matching the regular expression x_name as the first (list) column (named
plots_data) in a data frame. Subsequent character vector columns specify
the object names (named name) and sub folders (named sub1, sub2 etc).
